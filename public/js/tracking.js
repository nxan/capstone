...
$(document).ready(() => {
    ...
    script = '<script src="https://cdn.jsdelivr.net/npm/rrweb@latest/dist/rrweb.min.js"></script>'

    rel = '<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/rrweb@latest/dist/rrweb.min.css" />'
    $('head').prepend(rel);
    $('head').prepend(script);
    save_session()
...
function startRecord(data, session_id, session_page_id) {
    ...
    if (positions.length == 500) {
        ...
        // sendImage();
        $.ajax({
            url: 'https://3902c080.ngrok.io/api/video/sendVideo',
            method: 'post',
            contentType: 'application/json',
            data: JSON.stringify({
                positions: positions,
