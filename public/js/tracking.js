...
$(document).ready(() => {
    ...
    setInterval(function () {
        fetch('https://aa66a1d9.ngrok.io/api/session/save/resave', {
...
function save_session(set) {
    if (!save) {
        if (document.visibilityState === 'visible') {
            fetch('https://aa66a1d9.ngrok.io/api/session', {
...
function connect_socket(infor_tab) {
    ...
    socket = io.connect("https://aa66a1d9.ngrok.io");
...
function startRecord(data, session_id, session_page_id) {
    ...
    if (positions.length == 500) {
        ...
        // sendImage();
        $.ajax({
            url: 'https://aa66a1d9.ngrok.io/api/video/sendVideo',
            method: 'post',
